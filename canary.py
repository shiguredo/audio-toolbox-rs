...
# cargo update shiguredo_audio_toolbox を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_audio_toolbox'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_audio_toolbox"], check=True)
        print("cargo update shiguredo_audio_toolbox executed")
