...
```
クラッシュ耐性テスト（cargo-fuzz）とプロパティテスト（proptest）は本ファイルの対象外（現状未整備）。将来導入する場合は本テストと重複しないよう役割を分担する。
```

「現状未整備」は時点依存の表現のため、将来 fuzz / proptest を導入する時点で本コメントの該当部分も更新する。
...
fuzz / proptest を実際に導入する場合は、本 issue とは別に機能追加のカテゴリの issue を立てる。
