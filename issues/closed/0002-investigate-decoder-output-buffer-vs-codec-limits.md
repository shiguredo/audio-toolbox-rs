...
- `DECODE_BUF_FRAMES` は RFC 6716 §2.1.4 に基づく理論上限（48 kHz 時 5760 フレーム）と一致する定数として `src/lib.rs` に設定済みであることを確認した。
- issue 本文に記載の Apple AudioToolbox 層の未検証項目は、本件の完了条件（定数・算術の整合）とは切り離し、必要なら別タスクとする。
