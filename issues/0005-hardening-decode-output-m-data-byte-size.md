...
したがって **`size` が `pcm_buf.len()` を超える**と、`truncate` は **縮めない**ままとなり、**返却 `Vec` の `.len()` が「有効な PCM サンプル数」と一致しない**可能性がある（少なくとも末尾に **初期化ゼロのままの余剰要素**などが残り、他パターンもありうる。呼び出し側が「`len()` ＝有効サンプル数」と解釈すると **誤った長さ**になる）。
