...
Medium とする。crate 利用者が docs.rs でドキュメントを確認しても、README で紹介している API の利用方法の解説 (doc コメント) が表示されず、利用者体験を大きく損ねる。修正は Cargo.toml への追加と build.rs の DOCS_RS スタブ拡張で済む。

## 現状

- `src/lib.rs` の `impl Encoder` / `impl Decoder`、`src/codec_info.rs` の `supported_codecs()` は `#[cfg(target_os = "macos")]` で gate されている。
- docs.rs は既定で `x86_64-unknown-linux-gnu` でビルドするため、これらの `impl` / 関数は cfg が偽になりドキュメントに現れない (`Encoder` / `Decoder` 構造体と構造体レベルの doc コメントは表示されるが、メソッドと `supported_codecs()` は表示されない)。macOS gate されたメソッドへの intra-doc link も 11 箇所 unresolved になり、HTML 上では `[`Encoder::new()`]` のような生テキストとして表示される (develop 時点で `DOCS_RS=1 cargo doc --no-deps` を実行して確認済み)。docs.rs のランディングページのプラットフォーム一覧も x86_64-unknown-linux-gnu のみで、macOS ターゲットのドキュメントは生成されていない。
...
4. `CHANGES.md` の develop / `### misc` に [UPDATE] として追記され、追記エントリに issue 番号・issue ファイル名が含まれない。エントリは shiguredo-changelog スキルのフォーマット (担当者行 `- @ユーザー名` を含む) に従う。
