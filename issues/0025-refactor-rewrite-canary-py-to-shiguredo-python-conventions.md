...
`canary.py` を shiguredo-python 規約 (uv 管理、`ruff` / `ty`、`str | None`、`logging`、`__all__` 等) に沿って全面的に書き直す。合わせて事故防止のガード (作業ディレクトリの検証、Cargo.toml パースの堅牢化) を入れる。docstring は AGENTS.md の「コメントはしっかり入れること」に基づき日本語で付ける。
...
- `from typing import Optional` を使い `Optional[str]` を宣言している (`str | None` は PEP 604 で Python 3.10 以降から利用可能であり、サポート対象 3.12〜3.14 では `from __future__ import annotations` なしで使える)。
...
- `re.subn` で Cargo.toml を書き換えている (`tomllib` + `tomli-w` に置き換える。`tomli_w` はコメントを保持しないため、書き換え結果の検証が必要。現行 Cargo.toml にコメントは無い。issue 0027 が Cargo.toml に追加するコメントは canary 実行で失われるため、0027 側の完了条件 (canary リリース後の残存確認) で担保されている)。
...
加えてルートの `.gitignore` に `.venv/` (uv の仮想環境) や `.mypy_cache/` の除外が無く、uv 管理環境への移行時に生成物が誤ってコミットされる余地がある (`.mypy_cache/` は現在リポジトリに存在しない)。

## 完了条件

1. `pyproject.toml` と `uv.lock` が新設され、`uv sync` が成功する (`ls pyproject.toml uv.lock` が成功する)。
2. 型記法が `str | None` になる (`grep -nE "Optional|typing import" canary.py` が 0 件)。`from __future__ import annotations` は追加しない (PEP 604 が Python 3.10 以降で実行時利用可能なため不要。shiguredo-python 規約の「不要な場合は付けないこと」に従う)。
...
9. `.gitignore` に `.mypy_cache/` と `.venv/` が追加される (`grep -n "mypy_cache" .gitignore` が 1 件、`grep -n "\.venv" .gitignore` が 1 件)。
...
2. `Optional[str]` を `str | None` に置換し、`from typing import Optional` を削除する。`from __future__ import annotations` は不要 (PEP 604 は Python 3.10 以降で実行時利用可能) のため追加しない。
...
9. `.gitignore` に `.mypy_cache/` と `.venv/` を追加する (`.mypy_cache/` は現在リポジトリに存在しないため削除は不要)。
