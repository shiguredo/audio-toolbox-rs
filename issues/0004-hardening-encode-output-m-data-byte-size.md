...
- `mDataByteSize` が `ENCODE_BUF_SIZE` を超える場合、**`Result::Err`** 等で呼び出し側に伝える（または文書化した上で入力拒否）。
- 正常系では **`encoded_data[..size]` が常に境界内**になること（クランプだけで成功扱いにするかは設計判断。誤ったデータを黙って返さないこと）。
